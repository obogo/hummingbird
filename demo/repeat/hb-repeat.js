...
(function(exports, global) {
    ...
    define("consoleGraph", [ "apply" ], function(apply) {
        ...
        function graph(data, label, color) {
            ...
            context.fillText("  " + (len > 1 ? len + " / " + min + " / " + max : data[0]), 2, height * .75);
            ...
            return {
                link: function(scope, el, alias, attr) {
                    ...
                    function preRender(list, oldList) {
                        ...
                        currentList = list;
                        ar.setup(bottomUp ? UP : DOWN, topDown || bottomUp || 10, len);
                        ar.next();
                        render(list, oldList);
                    }
                    ...
                    function render(list, oldList) {
                        var i = 0, len, child, s, data;
                        if (list && (len = list.length)) {
                            removeUntil(len);
                            while (!ar.complete && !ar.atChunkEnd) {
                                child = el.children[ar.index];
                                if (!child) {
                                    data = {};
                                    data[itemName] = list[ar.index];
                                    data.$index = ar.index;
                                    s = scope.$new();
                                    child = $app.addChild(el, template, s, data, ar.direction === ar.up);
                                } else if (list[i]) {
                                    s = child.scope;
                                    s[itemName] = list[ar.index];
                                    s.$index = ar.index;
                                }
                                ar.inc();
                            }
 ...
 import hb.directive
 */
    define("module", [ "hb", "hb.compiler", "hb.scope", "hb.val", "injector", "interpolator", "removeHTMLComments", "each", "ready", "hb.debug" ], function(hb, compiler, scope, val, injector, interpolator, removeHTMLComments, each, ready, debug) {
        var modules = {};
