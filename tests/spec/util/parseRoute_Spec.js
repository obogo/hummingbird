hb.define('parseRouteSpec', ['parseRoute'], function (parseRoute) {
    describe('parseRoute', function () {
        ...
        it("should parse without a port", function() {
            var result = parseRoute.match('/services/login/:id', 'http://test.com/services/login/123');
            expect(!!result).toBe(true);
        });
        ...
        it("should match extract params", function() {
            var result = parseRoute.extractParams('/services/login/:id/:name', 'http://localhost:63342/services/login/123/name');
            expect(result.params).toEqual({id:'123', name:'name'});
        });

        it("should match extract query params", function() {
            var result = parseRoute.extractParams('/services/login/?a&b', 'http://localhost:63342/services/login?a=1&b=2');
            expect(result.query).toEqual({a:'1', b:'2'});
        });

        it("should match extract both params combined", function() {
            var result = parseRoute.extractParams('/services/login/:id/:name/?a&b', 'http://localhost:63342/services/login/123/name?a=1&b=2', true);
        ...
        it("should NOT find the pattern in the following url because it starts after the domain", function() {
            var result = parseRoute.match('/v1/config/:id', 'http://localhost:63342/blast/demo/v1/config/wes');
            expect(result).toBe(false);
        });

        it("should NOT find the pattern in the following url because it starts after the domain", function() {
            var result = parseRoute.match('/:id', 'http://localhost:63342/blast/demo/v1/config/wes');
            expect(result).toBe(false);
        });

        it("should find the pattern in the following url", function() {
            var result = parseRoute.match('/:id', 'http://localhost:63342/blast');
            expect(result).toBe(true);
        });
    });
});
